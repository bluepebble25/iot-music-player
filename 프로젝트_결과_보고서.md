...
<ul>휴 전구를 컨트롤 하기 위해서는 무선공유기와 브릿지를 연결해야 하는데, 브릿지가 인터넷망에 접속되지 않는 문제가 발생했다. 그 결과 전구를 실제로 작동시킬 수 없어 프로그램을 개발하는데 한계가 있었다.</ul></br> 
...
<ul> 머신러닝을 바탕으로 한 표정인식 기능에는
scikit-learn 이라는 패키지가 필요한데,
라즈베리환경에서는 설치가 안되는 문제가 발생했다.

먼저 pc 환경에서 개발을 하고 라즈베리파이
환경에 이식하려는 계획을 세웠지만,
패키지 문제를 해결하지 못하고 결국 윈도우 환경에서 웹캠으로 실행할 수 밖에 없었다.
 </ul></br></ol>
